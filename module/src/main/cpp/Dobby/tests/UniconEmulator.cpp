...
CapstoneDisassembler::CapstoneDisassembler(const std::string &arch, csh csh_) : arch_(arch),
                                                                                csh_(csh_) {
}
...
static void
hook_unmapped(uc_engine *uc, uc_mem_type type, uint64_t address, int size, int64_t value,
              void *user_data) {
...
UniconEmulator::UniconEmulator(const std::string &arch) {
    ...
    uc_hook_add(uc_, &hook_unmapped_handle, UC_HOOK_MEM_UNMAPPED, (void *) hook_unmapped, this, 1,
                0);
}
...
void check_insn_relo(char *buffer, size_t buffer_size, bool check_fault_addr, int check_reg_id,
    ...
    relo_ue->emulate(relocate_addr, relocate_addr + relo_stop_size, (char *) relocated->addr,
                     relocated->size);
