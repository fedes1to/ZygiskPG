...
static char *fast_get_shared_cache_path() {
    ...
    do {
        ...
        if (path != NULL) {
            ...
            path = IPHONE_DYLD_SHARED_CACHE_DIR
            ...
            path = MACOSX_MRM_DYLD_SHARED_CACHE_DIR
            DYLD_SHARED_CACHE_BASE_NAME
            "arm64";
            r = stat(path, &statbuf);
            if (r == 0) {
...
int shared_cache_ctx_init(shared_cache_ctx_t *ctx) {
    ...
    mmap_shared_cache =
            (struct dyld_cache_header *) mmap(0, mmap_length, PROT_READ, MAP_FILE | MAP_PRIVATE, fd,
                                              mmap_offset);
    ...
    mmap_shared_cache =
            (struct dyld_cache_header *) ((addr_t) mmap_shared_cache -
                                          runtime_shared_cache->localSymbolsOffset);
    ...
    const struct dyld_cache_mapping_info *mappings =
            (struct dyld_cache_mapping_info *) ((char *) runtime_shared_cache +
                                                runtime_shared_cache->mappingOffset);
    ...
    localInfo =
            (struct dyld_cache_local_symbols_info *) ((char *) mmap_shared_cache +
                                                      runtime_shared_cache->localSymbolsOffset);

    static struct dyld_cache_local_symbols_entry *localEntries = NULL;
    localEntries = (struct dyld_cache_local_symbols_entry *) ((char *) localInfo +
