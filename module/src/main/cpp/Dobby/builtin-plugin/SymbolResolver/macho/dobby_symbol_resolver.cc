...
uintptr_t
iterate_exported_symbol(mach_header_t *header, const char *symbol_name, uint64_t *out_flags) {
...
PUBLIC void *
DobbyMachOSymbolResolverOptions(void *header_, const char *symbol_name, bool add_slide) {
    ...
    result = iterate_symbol_table((char *) symbol_name, macho_ctx.symtab,
                                  macho_ctx.symtab_cmd->nsyms, macho_ctx.strtab);
    if (result) {
        result = result + slide;
        return (void *) result;
    }
...
PUBLIC void *DobbySymbolResolver(const char *image_name, const char *symbol_name_pattern) {
    ...
    for (auto iter = modules.begin(); iter != modules.end(); iter++) {
        ...
        memset(&macho_ctx, 0, sizeof(macho_ctx_t));
        macho_ctx_init(&macho_ctx, header);
        result = iterate_symbol_table((char *) symbol_name_pattern, macho_ctx.symtab,
                                      macho_ctx.symtab_cmd->nsyms,
                                      macho_ctx.strtab);
        if (result) {
            result = result + slide;
            break;
    ...
    if (image_name != NULL && strcmp(image_name, "dyld") == 0) {
        ...
        result =
                iterate_symbol_table((char *) symbol_name_pattern, dyld_ctx.symtab,
                                     dyld_ctx.symtab_cmd->nsyms, dyld_ctx.strtab);
        if (result) {
