...
PUBLIC void
common_closure_bridge_handler(DobbyRegisterContext *ctx, ClosureTrampolineEntry *entry) {
    DLOG(0, "common bridge handler: carry data: %p, carry handler: %p",
         (InterceptEntry *) entry->carry_data,
         entry->carry_handler);
