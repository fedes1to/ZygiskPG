...
void FunctionWrapperRouting::BuildPreCallRouting() {
    ...
    ClosureTrampolineEntry *cte = ClosureTrampoline::CreateClosureTrampoline(entry_,
                                                                             (void *) prologue_routing_dispatch);
    this->prologue_dispatch_bridge = cte->address;

    DLOG(0, "Create pre call closure trampoline to 'prologue_routing_dispatch' at %p",
         cte->address);
}
...
void FunctionWrapperRouting::BuildPostCallRouting() {
    ...
    // format trampoline
    closure_trampoline_entry = ClosureTrampoline::CreateClosureTrampoline(entry_,
                                                                          (void *) epilogue_routing_dispatch);
