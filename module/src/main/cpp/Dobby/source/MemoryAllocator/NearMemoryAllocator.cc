...
MemBlock *NearMemoryAllocator::allocateNearBlockFromDefaultAllocator(uint32_t size, addr_t pos,
                                                                     size_t search_range,
                                                                     bool executable) {
    ...
    auto allocateFromDefaultArena = [&](MemoryArena *arena, uint32_t size) -> addr_t {
        ...
        DLOG(0,
             "[near memory allocator] unused memory from default allocator %p(%p), within pos: %p, serach_range: %p",
    ...
    addr_t unused_mem = 0;
    if (executable) {
        for (auto iter = default_allocator->code_arenas.begin();
             iter != default_allocator->code_arenas.end(); iter++) {
            arena = *iter;
        ...
        for (auto iter = default_allocator->data_arenas.begin();
             iter != default_allocator->data_arenas.end(); iter++) {
            arena = *iter;
            unused_mem = allocateFromDefaultArena(arena, size);
            if (unused_mem)
                continue;
...
MemBlock *NearMemoryAllocator::allocateNearBlockFromUnusedRegion(uint32_t size, addr_t pos,
                                                                 size_t search_range,
                                                                 bool executable) {
    ...
    auto check_has_sufficient_memory_between_region = [&](MemRegion region, MemRegion next_region,
        ...
        DLOG(0,
             "[near memory allocator] unused memory from unused region %p(%p), within pos: %p, serach_range: %p",
             unused_mem_start, size, pos, search_range);
        return unused_mem_start;
    };
    ...
    auto unused_arena_size =
            unused_arena_end_page_addr - unused_arena_first_page_addr + OSMemory::PageSize();
    ...
    auto register_near_arena = [&](addr_t arena_addr, size_t arena_size) -> MemoryArena * {
        ...
        OSMemory::SetPermission((void *) arena->addr, arena->size,
                                executable ? kReadExecute : kReadWrite);
        return arena;
    };
...
MemBlock *NearMemoryAllocator::allocateNearBlock(uint32_t size, addr_t pos, size_t search_range,
                                                 bool executable) {
    ...
    if (!result) {
        ERROR_LOG("[near memory allocator] allocate near block failed (%p, %p, %p)", size, pos,
                  search_range);
    }
    return result;
}
...
uint8_t *
NearMemoryAllocator::allocateNearExecMemory(uint32_t size, addr_t pos, size_t search_range) {
    auto block = allocateNearBlock(size, pos, search_range, true);
    if (!block)
        return nullptr;

    DLOG(0, "[near memory allocator] allocate exec memory at: %p, size: %p", block->addr,
         block->size);
    return (uint8_t *) block->addr;
}

uint8_t *
NearMemoryAllocator::allocateNearExecMemory(uint8_t *buffer, uint32_t buffer_size, addr_t pos,
                                            size_t search_range) {
...
uint8_t *
NearMemoryAllocator::allocateNearDataMemory(uint32_t size, addr_t pos, size_t search_range) {
    auto block = allocateNearBlock(size, pos, search_range, false);
    if (!block)
        return nullptr;

    DLOG(0, "[near memory allocator] allocate data memory at: %p, size: %p", block->addr,
         block->size);
    return (uint8_t *) block->addr;
}

uint8_t *
NearMemoryAllocator::allocateNearDataMemory(uint8_t *buffer, uint32_t buffer_size, addr_t pos,
                                            size_t search_range) {
    auto mem = allocateNearExecMemory(buffer_size, pos, search_range);
