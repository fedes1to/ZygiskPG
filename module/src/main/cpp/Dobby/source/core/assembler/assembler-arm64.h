...
namespace zz {
    namespace arm64 {
        ...
        constexpr Register
        TMP_REG_0 = X(ARM64_TMP_REG_NDX_0);
...
// ---
        ...
        public:
            inline explicit MemOperand(Register base, int64_t offset = 0,
                    ...
                    : base_(base), regoffset_(InvalidRegister), offset_(offset),
                      addrmode_(addrmode), shift_(NO_SHIFT),
            ...
            inline explicit MemOperand(Register base, Register regoffset, Shift shift = LSL,
                                       unsigned shift_imm = 0)
                    : base_(base), regoffset_(regoffset), offset_(0), addrmode_(Offset),
...
// ---
        ...
        public:
            ...
            // LogicalImmeidate
            static int32_t
            EncodeLogicalImmediate(const Register &rd, const Register &rn, const Operand &operand) {
            ...
            // LogicalShift
            static int32_t
            EncodeLogicalShift(const Register &rd, const Register &rn, const Operand &operand) {
                return (sf(rd) | shift(operand.shift()) | Rm(operand.reg()) |
                        LeftShift(operand.shift_extend_imm(), 6, 10) |
                        Rn(rn) | Rd(rd));
            }

            // LoadStore
            static int32_t LoadStorePair(LoadStorePairOp op, CPURegister rt, CPURegister rt2,
        ...
        private:
            ...
            void LoadStore(LoadStoreOp op, CPURegister rt, const MemOperand &addr) {
                ...
                if (addr.IsImmediateOffset()) {
                    ...
                    Emit(LoadStoreUnsignedOffsetFixed | op | LeftShift(imm12, 12, 10) |
                         Rn(addr.base()) | Rt(rt));
            ...
            void LoadStorePair(LoadStorePairOp op, CPURegister rt, CPURegister rt2,
                               const MemOperand &addr) {
                int32_t combine_fields_op =
                        OpEncode::LoadStorePair(op, rt, rt2, addr) | Rt2(rt2) | Rn(addr.base()) |
                        Rt(rt);
                int32_t addrmodeop;
            ...
            void MoveWide(Register rd, uint64_t imm, int shift, MoveWideImmediateOp op) {
                ...
                Emit(MoveWideImmediateFixed | op | OpEncode::sf(rd) | LeftShift(shift, 2, 21) |
                     imm16 | Rd(rd));
            }

            void AddSubImmediate(const Register &rd, const Register &rn, const Operand &operand,
            ...
            void
            Logical(const Register &rd, const Register &rn, const Operand &operand, LogicalOp op) {
                if (operand.IsImmediate()) {
            ...
            void LogicalImmediate(const Register &rd, const Register &rn, const Operand &operand,
                                  LogicalOp op) {
                int32_t combine_fields_op = OpEncode::EncodeLogicalImmediate(rd, rn, operand);
                Emit(op | combine_fields_op);
            }

            void LogicalShift(const Register &rd, const Register &rn, const Operand &operand,
                              LogicalOp op) {
