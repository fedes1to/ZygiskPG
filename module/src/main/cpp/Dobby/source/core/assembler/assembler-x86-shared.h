        ...
        public:
            ...
            ~AssemblerPseudoLabel(void) {
                for (size_t i = 0; i < instructions_.getCount(); i++) {
                    PseudoLabelInstruction *item = (PseudoLabelInstruction *) instructions_.getObject(
                            i);
                    delete item;
                }
            ...
            void link_confused_instructions(CodeBuffer *buffer = nullptr) {
                ...
                for (size_t i = 0; i < instructions_.getCount(); i++) {
                    PseudoLabelInstruction *instruction = (PseudoLabelInstruction *) instructions_.getObject(
                            i);
...
// Assembler
        ...
        public:
            Assembler(void *address, int mode) : AssemblerBase(address)
        ...
        public:
            ...
            void EmitREX_ExtraRegister(Register reg) {
                uint8_t rex = EmitOptionalRex(false, reg.size() == 64, reg.code() > 7, false,
                                              reg.code() > 7);
                if (rex)
                    Emit1(rex);
            }
            ...
            void
            Emit_OpEn_MemOperand_Immediate(uint8_t extra_opcode, Address &operand, Immediate imm) {
            }
            ...
            void PseudoBind(AssemblerPseudoLabel *label) {
                ...
                if (label->has_confused_instructions()) {
                    label->link_confused_instructions(
                            reinterpret_cast<CodeBuffer *>(this->GetCodeBuffer()));
                }
            }
