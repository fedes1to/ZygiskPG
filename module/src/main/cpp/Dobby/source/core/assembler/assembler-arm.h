...
// ---
        ...
        public:
            Operand(int immediate) : imm_(immediate), rm_(no_reg), shift_(LSL), shift_imm_(0),
                                     rs_(no_reg) {
            }
            ...
            Operand(Register rm, Shift shift, Register rs) : imm_(0), rm_(rm), shift_(shift),
                                                             shift_imm_(0), rs_(rs) {
            }
...
// ---
        ...
        public:
            MemOperand(Register rn, int32_t offset = 0, AddrMode addrmode = Offset)
                    : rn_(rn), offset_(offset), rm_(no_reg), shift_(LSL), shift_imm_(0),
                      addrmode_(addrmode) {
            }

            MemOperand(Register rn, Register rm, AddrMode addrmode = Offset)
                    : rn_(rn), offset_(0), rm_(rm), shift_(LSL), shift_imm_(0),
                      addrmode_(addrmode) {
            }

            MemOperand(Register rn, Register rm, Shift shift, uint32_t shift_imm,
                       AddrMode addrmode = Offset)
                    : rn_(rn), offset_(0), rm_(rm), shift_(shift), shift_imm_(shift_imm),
                      addrmode_(addrmode) {
            }
...
// ---
        ...
        public:
            ...
            void add_sub(uint32_t encoding, Condition cond, Register rd, Register rn,
            ...
            void
            load_store(uint32_t encoding, Condition cond, Register rt, const MemOperand &operand) {
                encoding |= (cond << kConditionShift);
...
// ---
        ...
        public:
            ...
            void CallFunction(ExternalReference function) {
                ...
                buffer_->Emit32((uint32_t)(uintptr_t)
                function.address());
            }
