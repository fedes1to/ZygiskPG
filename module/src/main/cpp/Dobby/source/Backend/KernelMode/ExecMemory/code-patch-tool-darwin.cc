...
DobbyCodePatch(void *address, uint8_t *buffer, uint32_t buffer_size) {
...

    {
        ...
        pmap_kit_set_perm(kernel_pmap, (vaddr_t) address, (vaddr_t) address + PAGE_SIZE,
    ...
    if (0) {
        ...
        // change permission
        kr = mach_vm_protect(self_task, remap_dummy_page, page_size, false,
                             VM_PROT_READ | VM_PROT_EXECUTE);
        KERN_RETURN_ERROR(kr, kMemoryOperationError);

        static boolean_t (*vm_map_lookup_entry)(vm_map_t map, vm_map_offset_t address,
                                                vm_map_entry_t *entry) = nullptr;
        if (vm_map_lookup_entry == nullptr)
            vm_map_lookup_entry = (typeof(vm_map_lookup_entry)) DobbySymbolResolver(nullptr,
        ...
        kr = mach_vm_remap(self_task, &remap_dest_page, page_size, 0,
                           VM_FLAGS_OVERWRITE | VM_FLAGS_FIXED, self_task,
                           remap_dummy_page, TRUE, &curr_protection, &max_protection,
                           VM_INHERIT_COPY);
        KERN_RETURN_ERROR(kr, kMemoryOperationError);
