...
void *OSMemory::Allocate(size_t size, MemoryPermission access, void *fixed_address) {
    ...
    if (prot & PROT_EXEC || prot == PROT_NONE) {
        ...
        kern_return_t ret = mach_vm_allocate(kernel_map, (mach_vm_address_t * ) & addr, size,
                                             flags);
        if (ret != KERN_SUCCESS) {
            panic("mach_vm_allocate");
            return nullptr;
        }
        ret = vm_map_wire(kernel_map, (mach_vm_address_t) addr, (mach_vm_address_t) addr + size,
                          PROT_NONE, false);
        if (ret != KERN_SUCCESS) {
            panic("vm_map_wire");
            return nullptr;
