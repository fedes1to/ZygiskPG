...
static inline vm_map_offset_t vme_start(vm_map_entry_t entry) {
    ...
    return ((vm_map_header *) ((addr_t) entry +
                               KERN_STRUCT_OFFSET(vm_map_entry, links)))->links.start;
}
...
const std::vector <RuntimeModule> *ProcessRuntimeUtility::GetProcessModuleMap() {
    ...
    if (kernel_base == nullptr) {
        ...
        _gLoadedKextSummariesPtr = (typeof(_gLoadedKextSummariesPtr)) DobbyMachOSymbolResolver(
                kernel_base, "_gLoadedKextSummaries");
