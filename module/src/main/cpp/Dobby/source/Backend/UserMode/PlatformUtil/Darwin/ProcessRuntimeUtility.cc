...
const std::vector <MemRegion> &ProcessRuntimeUtility::GetProcessMemoryLayout() {
    ...
    while (true) {
        ...
        kern_return_t kr = mach_vm_region_recurse(mach_task_self(), (mach_vm_address_t * ) & addr,
                                                  (mach_vm_size_t * ) & size, &depth,
                                                  (vm_region_recurse_info_t) & region_submap_info,
                                                  &count);
        if (kr != KERN_SUCCESS) {
        ...
        if (region_submap_info.is_submap) {
            ...
            if ((region_submap_info.protection & PROT_READ) &&
                (region_submap_info.protection & PROT_WRITE)) {
                permission = MemoryPermission::kReadWrite;
            } else if ((region_submap_info.protection & PROT_READ) ==
                       region_submap_info.protection) {
                permission = MemoryPermission::kRead;
            } else if ((region_submap_info.protection & PROT_READ) &&
                       (region_submap_info.protection & PROT_EXEC)) {
