...
#define ___ rip_insn_seq_buffer.
            ...
            if (insn.immediate_offset) {
                ___ EmitBuffer((buffer_cursor + insn.immediate_offset),
                               insn.length - insn.immediate_offset);
            }

            // jmp *(rip) => back to relo process
            codegen_x64_jmp_absolute_addr(&rip_insn_seq_buffer, curr_relo_ip);

            DobbyCodePatch((void *) rip_insn_seq, rip_insn_seq_buffer.GetBuffer(),
                           rip_insn_seq_buffer.GetBufferSize());
        }
...
#endif
    } else if (insn.primary_opcode >= 0xE0 &&
               insn.primary_opcode <= 0xE2) { // LOOPNZ/LOOPZ/LOOP/JECXZ
        // LOOP/LOOPcc
        UNIMPLEMENTED();
...
void
GenRelocateCodeX86Shared(void *buffer, CodeMemBlock *origin, CodeMemBlock *relocated, bool branch) {
    int expected_relocated_mem_size = 32;
    x86_try_again:
    if (!relocated->addr) {
        auto relocated_mem = MemoryAllocator::SharedAllocator()->allocateExecMemory(
                expected_relocated_mem_size);
