...
static void
init_reader(x86_insn_reader_t *rd, const unsigned char *begin, const unsigned char *buffer_cursor) {
...
static uint32_t read_imm(x86_insn_reader_t *rd, int size) {
    ...
    return (size == 8) ? read_byte(rd) : (size == 16) ? read_word(rd) : (size == 32) ? read_dword(
            rd) : 0;
}
