...
bool ImGui::BeginTableEx(const char *name, ImGuiID id, int columns_count, ImGuiTableFlags flags,
    ...
    const int instance_no = (table->LastFrameActive != g.FrameCount) ? 0 : table->InstanceCurrent +
                                                                           1;
    ...
    const bool pad_outer_x = (flags & ImGuiTableFlags_NoPadOuterX) ? false : (flags &
    ...
    const float inner_spacing_explicit = (pad_inner_x &&
                                          (flags & ImGuiTableFlags_BordersInnerV) == 0)
    ...
    table->InnerClipRect = (inner_window == outer_window) ? table->WorkRect
                                                          : inner_window->ClipRect;
    table->InnerClipRect.ClipWith(table->WorkRect);     // We need this to honor inner_width
    table->InnerClipRect.ClipWithFull(table->HostClipRect);
    table->InnerClipRect.Max.y = (flags & ImGuiTableFlags_NoHostExtendY) ? ImMin(
            table->InnerClipRect.Max.y, inner_window->WorkRect.Max.y)
...
void ImGui::TableUpdateLayout(ImGuiTable *table) {
    ...
    table->MinColumnWidth = ImMax(1.0f, g.Style.FramePadding.x *
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
        ...
        const bool start_auto_fit = (column->Flags & ImGuiTableColumnFlags_WidthFixed) ? (
                column->WidthRequest < 0.0f) : (column->StretchWeight < 0.0f);
        if (start_auto_fit)
    ...
    const float width_avail = ((table->Flags & ImGuiTableFlags_ScrollX) &&
                               table->InnerWidth == 0.0f) ? table->InnerClipRect.GetWidth()
                                                          : work_rect.GetWidth();
    ...
    const ImRect mouse_hit_rect(table->OuterRect.Min.x, table->OuterRect.Min.y,
                                table->OuterRect.Max.x, ImMax(table->OuterRect.Max.y,
...
void ImGui::TableUpdateBorders(ImGuiTable *table) {
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
        ...
        const float border_y2_hit = (table->Flags & ImGuiTableFlags_NoBordersInBody) ? hit_y2_head
                                                                                     : hit_y2_body;
...
void ImGui::EndTable() {
    ...
    if (table->Flags & ImGuiTableFlags_NoHostExtendX) {
        ...
        outer_window->DC.CursorMaxPos.x = ImMax(backup_outer_max_pos.x, table->OuterRect.Min.x +
                                                                        table->ColumnsAutoFitWidth);
    ...
    temp_data = (--g.TablesTempDataStacked > 0) ? &g.TablesTempData[g.TablesTempDataStacked - 1]
                                                : NULL;
...
void
ImGui::TableSetupColumn(const char *label, ImGuiTableColumnFlags flags, float init_width_or_weight,
                        ImGuiID user_id) {
    ImGuiContext &g = *GImGui;
    ImGuiTable *table = g.CurrentTable;
    ...
    if (table->IsInitializing) {
        ...
        if (column->WidthRequest < 0.0f && column->StretchWeight < 0.0f) {
            ...
            if (flags & ImGuiTableColumnFlags_WidthStretch)
                column->StretchWeight = (init_width_or_weight > 0.0f) ? init_width_or_weight
                                                                      : -1.0f;
...
void ImGui::TableSetupScrollFreeze(int columns, int rows) {
    ...
    table->FreezeColumnsRequest = (table->Flags & ImGuiTableFlags_ScrollX)
                                  ? (ImGuiTableColumnIdx) ImMin(columns, table->ColumnsCount) : 0;
    table->FreezeColumnsCount = (table->InnerWindow->Scroll.x != 0.0f) ? table->FreezeColumnsRequest
                                                                       : 0;
    table->FreezeRowsRequest = (table->Flags & ImGuiTableFlags_ScrollY) ? (ImGuiTableColumnIdx) rows
                                                                        : 0;
...
void ImGui::TableEndRow(ImGuiTable *table) {
    ...
    if (unfreeze_rows_actual) {
        ...
        table->BgClipRect.Min.y = table->Bg2ClipRectForDrawCmd.Min.y = ImMin(y0,
...
float ImGui::TableGetMaxColumnWidth(const ImGuiTable *table, int column_n) {
    ...
    const float min_column_distance =
            table->MinColumnWidth + table->CellPaddingX * 2.0f + table->CellSpacingX1 +
            table->CellSpacingX2;
    if (table->Flags & ImGuiTableFlags_ScrollX) {
...
void ImGui::TableSetColumnWidth(int column_n, float width) {
    ...
    ImGuiTableColumn *column_1 = (column_0->NextEnabledColumn != -1)
                                 ? &table->Columns[column_0->NextEnabledColumn] : NULL;
    ...
    if (column_1 == NULL)
        column_1 = (column_0->PrevEnabledColumn != -1)
                   ? &table->Columns[column_0->PrevEnabledColumn] : NULL;
    if (column_1 == NULL)
        return;
...
void ImGui::TableSetColumnWidthAutoAll(ImGuiTable *table) {
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
        ImGuiTableColumn *column = &table->Columns[column_n];
        if (!column->IsEnabled && !(column->Flags &
...
void ImGui::TableSetupDrawChannels(ImGuiTable *table) {
    ...
    const int channels_for_row = (table->Flags & ImGuiTableFlags_NoClip) ? 1
                                                                         : table->ColumnsEnabledCount;
    const int channels_for_bg = 1 + 1 * freeze_row_multiplier;
    const int channels_for_dummy = (table->ColumnsEnabledCount < table->ColumnsCount ||
                                    table->VisibleMaskByIndex != table->EnabledMaskByIndex) ? +1
                                                                                            : 0;
    ...
    table->Bg2DrawChannelUnfrozen = (ImGuiTableDrawChannelIdx) ((table->FreezeRowsCount > 0) ? 2 +
                                                                                               channels_for_row
...
void ImGui::TableDrawContextMenu(ImGuiTable *table) {
    ...
    if (table->Flags & ImGuiTableFlags_Hideable) {
        ...
        for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++) {
            ...
            bool menu_item_active = (other_column->Flags & ImGuiTableColumnFlags_NoHide) ? false
                                                                                         : true;
...
void ImGui::TableSaveSettings(ImGuiTable *table) {
    ...
    for (int n = 0; n < table->ColumnsCount; n++, column++, column_settings++) {
        const float width_or_weight = (column->Flags & ImGuiTableColumnFlags_WidthStretch)
                                      ? column->StretchWeight : column->WidthRequest;
    ...
    for (ImGuiTableSettings *settings = g.SettingsTables.begin();
        ...
        for (int column_n = 0; column_n < settings->ColumnsCount; column_n++, column++) {
            ...
            if (save_sort && column->SortOrder != -1)
                buf->appendf(" Sort=%d%c", column->SortOrder, (column->SortDirection ==
                                                               ImGuiSortDirection_Ascending) ? 'v'
                                                                                             : '^');
            buf->append("\n");
        }
        buf->append("\n");
...
void ImGui::DebugNodeTableSettings(ImGuiTableSettings *settings) {
    ...
    for (int n = 0; n < settings->ColumnsCount; n++) {
        ...
        BulletText("Column %d Order %d SortOrder %d %s Vis %d %s %7.3f UserID 0x%08X",
                   ...
                   (sort_dir == ImGuiSortDirection_Ascending) ? "Asc" : (sort_dir ==
                                                                         ImGuiSortDirection_Descending)
                                                                        ? "Des" : "---",
...
void ImGui::SetWindowClipRectBeforeSetChannel(ImGuiWindow *window, const ImRect &clip_rect) {
    ...
    window->DrawList->_ClipRectStack.Data[window->DrawList->_ClipRectStack.Size -
                                          1] = clip_rect_vec4;
}
...
void ImGui::EndColumns() {
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems) {
        ...
        for (int n = 1; n < columns->Count; n++) {
            ...
            // Draw column
            const ImU32 col = GetColorU32(
                    held ? ImGuiCol_SeparatorActive : hovered ? ImGuiCol_SeparatorHovered
                                                              : ImGuiCol_Separator);
