...
void ImGui::TextEx(const char *text, const char *text_end, ImGuiTextFlags flags) {
    ...
    if (text_end - text > 2000 && !wrap_enabled) {
        ...
        const float wrap_width = wrap_enabled ? CalcWrapWidthForPos(window->DC.CursorPos,
                                                                    wrap_pos_x) : 0.0f;
...
void ImGui::LabelTextV(const char *label, const char *fmt, va_list args) {
    ...
    const ImRect total_bb(pos, pos + ImVec2(w + (label_size.x > 0.0f ? style.ItemInnerSpacing.x +
                                                                       label_size.x : 0.0f),
    ...
    if (hovered) {
        if (!(flags & ImGuiButtonFlags_NoKeyModifiers) ||
            ...
            if ((flags & ImGuiButtonFlags_MouseButtonLeft) &&
                g.IO.MouseClicked[0]) { mouse_button_clicked = 0; }
            else if ((flags & ImGuiButtonFlags_MouseButtonRight) &&
                     g.IO.MouseClicked[1]) { mouse_button_clicked = 1; }
            else if ((flags & ImGuiButtonFlags_MouseButtonMiddle) &&
                     g.IO.MouseClicked[2]) { mouse_button_clicked = 2; }
            if ((flags & ImGuiButtonFlags_MouseButtonLeft) &&
                g.IO.MouseReleased[0]) { mouse_button_released = 0; }
            else if ((flags & ImGuiButtonFlags_MouseButtonRight) &&
...
bool ImGui::ButtonEx(const char *label, const ImVec2 &size_arg, ImGuiButtonFlags flags) {
    ...
    const ImU32 col = GetColorU32(
            (held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered
                                                                : ImGuiCol_Button);
...
bool ImGui::ArrowButtonEx(const char *str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags) {
    ...
    RenderArrow(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f),
...
void
ImGui::Image(ImTextureID user_texture_id, const ImVec2 &size, const ImVec2 &uv0, const ImVec2 &uv1,
...
bool
ImGui::SplitterBehavior(const ImRect &bb, ImGuiID id, ImGuiAxis axis, float *size1, float *size2,
...
bool ImGui::BeginCombo(const char *label, const char *preview_value, ImGuiComboFlags flags) {
    ...
    const ImRect total_bb(bb.Min, bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x +
                                                                        label_size.x : 0.0f, 0.0f));
    ItemSize(total_bb, style.FramePadding.y);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect &bb, ImGuiComboFlags flags) {
    ...
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint) {
        ...
        SetNextWindowSizeConstraints(ImVec2(w, 0.0f), ImVec2(FLT_MAX,
...
bool
ImGui::Combo(const char *label, int *current_item, bool (*items_getter)(void *, int, const char **),
...
int
ImGui::DataTypeFormatString(char *buf, int buf_size, ImGuiDataType data_type, const void *p_data,
                            const char *format) {
...
static float GetMinimumStepAtDecimalPrecision(int decimal_precision) {
    static const float min_steps[10] = {1.0f, 0.1f, 0.01f, 0.001f, 0.0001f, 0.00001f, 0.000001f,
                                        0.0000001f, 0.00000001f, 0.000000001f};
    if (decimal_precision < 0)
        return FLT_MIN;
    return (decimal_precision < IM_ARRAYSIZE(min_steps)) ? min_steps[decimal_precision] : ImPow(
            10.0f, (float) -decimal_precision);
}
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE *v, float v_speed, const TYPE v_min,
    ...
    if (is_logarithmic) {
        ...
        float v_old_parametric = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, v_cur,
                                                                                   v_min, v_max,
                                                                                   is_logarithmic,
                                                                                   logarithmic_zero_epsilon,
                                                                                   zero_deadzone_halfsize);
        float v_new_parametric = v_old_parametric + g.DragCurrentAccum;
        v_cur = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, v_new_parametric,
                                                                  v_min, v_max, is_logarithmic,
                                                                  logarithmic_zero_epsilon,
                                                                  zero_deadzone_halfsize);
    ...
    if (is_logarithmic) {
        // Convert to parametric space, apply delta, convert back
        float v_new_parametric = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, v_cur,
                                                                                   v_min, v_max,
                                                                                   is_logarithmic,
                                                                                   logarithmic_zero_epsilon,
                                                                                   zero_deadzone_halfsize);
...
bool ImGui::DragScalar(const char *label, ImGuiDataType data_type, void *p_data, float v_speed,
    ...
    const ImRect total_bb(frame_bb.Min, frame_bb.Max +
                                        ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x +
                                                                     label_size.x : 0.0f, 0.0f));
    ...
    if (!temp_input_is_active) {
        ...
        if (g.IO.ConfigDragClickToInputText && temp_input_allowed && !temp_input_is_active)
            if (g.ActiveId == id && hovered && g.IO.MouseReleased[0] && !IsMouseDragPastThreshold(0,
    ...
    const ImU32 frame_col = GetColorU32(
            g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered
                                                                : ImGuiCol_FrameBg);
...
bool
ImGui::DragFloatRange2(const char *label, float *v_current_min, float *v_current_max, float v_speed,
...
bool
ImGui::DragInt(const char *label, int *v, float v_speed, int v_min, int v_max, const char *format,
               ImGuiSliderFlags flags) {
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max,
    ...
    if (is_logarithmic) {
        ...
        if (t <= 0.0f)
            ...
            {
                float zero_point_center = (-(float) ImMin(v_min, v_max)) / ImAbs((float) v_max -
        ...
        if (is_floating_point) {
            ...
            if (t < 1.0) {
                ...
                result = (TYPE) ((SIGNEDTYPE) v_min + (SIGNEDTYPE) (v_new_off_f +
...
bool ImGui::SliderBehaviorT(const ImRect &bb, ImGuiID id, ImGuiDataType data_type, TYPE *v,
    ...
    if (g.ActiveId == id) {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
            ...
            if (input_delta != 0.0f) {
                const int decimal_precision = is_floating_point ? ImParseFormatPrecision(format, 3)
                                                                : 0;
                if (decimal_precision > 0) {
            ...
            if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
                ...
                clicked_t = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, *v, v_min,
        ...
        if (set_new_value) {
            TYPE v_new = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, clicked_t,
                                                                           v_min, v_max,
                                                                           is_logarithmic,
                                                                           logarithmic_zero_epsilon,
                                                                           zero_deadzone_halfsize);
    ...
    if (slider_sz < 1.0f) {
        *out_grab_bb = ImRect(bb.Min, bb.Min);
        ...
        float grab_t = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, *v, v_min,
                                                                         v_max, is_logarithmic,
                                                                         logarithmic_zero_epsilon,
                                                                         zero_deadzone_halfsize);
        if (axis == ImGuiAxis_Y)
    ...
    switch (data_type) {
        ...
        case ImGuiDataType_Double:
            ...
            return SliderBehaviorT<double, double, double>(bb, id, data_type, (double *) p_v,
                                                           *(const double *) p_min,
                                                           *(const double *) p_max, format, flags,
                                                           out_grab_bb);
        case ImGuiDataType_COUNT:
            break;
    }
...
bool
ImGui::SliderScalar(const char *label, ImGuiDataType data_type, void *p_data, const void *p_min,
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(
...
bool
ImGui::SliderFloat2(const char *label, float v[2], float v_min, float v_max, const char *format,
                    ImGuiSliderFlags flags) {
    return SliderScalarN(label, ImGuiDataType_Float, v, 2, &v_min, &v_max, format, flags);
}

bool
ImGui::SliderFloat3(const char *label, float v[3], float v_min, float v_max, const char *format,
                    ImGuiSliderFlags flags) {
    return SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max, format, flags);
}

bool
ImGui::SliderFloat4(const char *label, float v[4], float v_min, float v_max, const char *format,
                    ImGuiSliderFlags flags) {
...
bool
ImGui::VSliderScalar(const char *label, const ImVec2 &size, ImGuiDataType data_type, void *p_data,
                     const void *p_min, const void *p_max, const char *format,
                     ImGuiSliderFlags flags) {
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems)
...
bool
ImGui::TempInputScalar(const ImRect &bb, ImGuiID id, const char *label, ImGuiDataType data_type,
...
bool
ImGui::InputScalar(const char *label, ImGuiDataType data_type, void *p_data, const void *p_step,
...
bool
ImGui::InputDouble(const char *label, double *v, double step, double step_fast, const char *format,
                   ImGuiInputTextFlags flags) {
    flags |= ImGuiInputTextFlags_CharsScientific;
...
namespace ImStb {
    ...
    static ImWchar
    STB_TEXTEDIT_GETCHAR(const ImGuiInputTextState *obj, int idx) { return obj->TextW[idx]; }
...
#define STB_TEXTEDIT_MOVEWORDRIGHT  STB_TEXTEDIT_MOVEWORDRIGHT_WIN
    ...
    static bool STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState *obj, int pos, const ImWchar *new_text,
        ...
        const int new_text_len_utf8 = ImTextCountUtf8BytesFromStr(new_text,
...
ImGuiInputTextCallbackData::InsertChars(int pos, const char *new_text, const char *new_text_end) {
    ...
    const int new_text_len = new_text_end ? (int) (new_text_end - new_text) : (int) strlen(
            new_text);
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf, int buf_size,
    ...
    IM_ASSERT(!((flags & ImGuiInputTextFlags_CallbackCompletion) && (flags &
    ...
    if (is_multiline) {
        ...
        PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0,
    ...
    const bool input_requested_by_nav = (g.ActiveId != id) && ((g.NavActivateInputId == id) ||
                                                               (g.NavActivateId == id &&
                                                                g.NavInputSource ==
    ...
    if (g.ActiveId == id) {
        ...
        const float mouse_y = (is_multiline ? (io.MousePos.y - draw_window->DC.CursorPos.y) : (
                g.FontSize * 0.5f));
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        const bool is_shortcut_key = g.IO.ConfigMacOSXBehaviors ? (io.KeyMods ==
        ...
        const bool is_paste = ((is_shortcut_key && IsKeyPressed(ImGuiKey_V)) ||
                              ...
                              !is_readonly;
        const bool is_undo = ((is_shortcut_key && IsKeyPressed(ImGuiKey_Z)) && !is_readonly &&
                              is_undoable);
        ...
        const bool is_cancel = IsKeyPressed(ImGuiKey_Escape) ||
        ...
        if (IsKeyPressed(ImGuiKey_LeftArrow)) {
            ...
            state->OnKeyPressed(
                    (is_startend_key_down ? STB_TEXTEDIT_K_LINEEND : is_wordmove_key_down
            ...
            state->OnKeyPressed(
                    io.KeyCtrl ? STB_TEXTEDIT_K_TEXTSTART | k_mask : STB_TEXTEDIT_K_LINESTART |
                                                                     k_mask);
        }
    ...
    if (render_cursor || render_selection) {
        ...
        if (render_cursor) {
            ...
            bool cursor_is_visible =
                    (!g.IO.ConfigInputTextCursorBlink) || (state->CursorAnim <= 0.0f) ||
...
bool ImGui::ColorEdit4(const char *label, float col[4], ImGuiColorEditFlags flags) {
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview)) {
        const float button_offset_x = ((flags & ImGuiColorEditFlags_NoInputs) ||
                                       (style.ColorButtonPosition == ImGuiDir_Left)) ? 0.0f :
...
static void
RenderArrowsForVerticalBar(ImDrawList *draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w,
                           float alpha) {
...
bool ImGui::ColorPicker4(const char *label, float col[4], ImGuiColorEditFlags flags,
    ...
    IM_ASSERT(ImIsPowerOfTwo(
            flags & ImGuiColorEditFlags_PickerMask_)); // Check that only 1 is selected
    IM_ASSERT(ImIsPowerOfTwo(
            flags & ImGuiColorEditFlags_InputMask_));  // Check that only 1 is selected
    if (!(flags & ImGuiColorEditFlags_NoOptions))
        ...
        if (IsItemActive()) {
            ...
            if (ImTriangleContainsPoint(triangle_pa, triangle_pb, triangle_pc,
                ...
                if (!ImTriangleContainsPoint(triangle_pa, triangle_pb, triangle_pc,
                    ...
                    current_off_unrotated = ImTriangleClosestPoint(triangle_pa, triangle_pb,
                                                                   triangle_pc,
                                                                   current_off_unrotated);
                float uu, vv, ww;
    ...
    const ImU32 col_hues[
            6 + 1] = {IM_COL32(255, 0, 0, style_alpha8), IM_COL32(255, 255, 0, style_alpha8),
...
bool
ImGui::ColorButton(const char *desc_id, const ImVec4 &col, ImGuiColorEditFlags flags, ImVec2 size) {
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f) {
        ...
        ImVec4 col_source = (flags & ImGuiColorEditFlags_AlphaPreview) ? col_rgb
    ...
    if (!(flags & ImGuiColorEditFlags_NoTooltip) && hovered)
        ColorTooltip(desc_id, &col.x, flags & (ImGuiColorEditFlags_InputMask_ |
...
void ImGui::ColorEditOptionsPopup(const float *col, ImGuiColorEditFlags flags) {
    ...
    if (Button("Copy as..", ImVec2(-1, 0)))
        OpenPopup("Copy");
    if (BeginPopup("Copy")) {
        int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(
                col[1]), cb = IM_F32_TO_INT8_SAT(col[2]), ca = (flags & ImGuiColorEditFlags_NoAlpha)
                                                               ? 255 : IM_F32_TO_INT8_SAT(col[3]);
        char buf[64];
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char *label,
    ...
    const ImVec2 padding = (display_frame || (flags & ImGuiTreeNodeFlags_FramePadding))
                           ? style.FramePadding : ImVec2(style.FramePadding.x,
    ...
    const float text_offset_x = g.FontSize + (display_frame ? padding.x * 3 : padding.x *
    ...
    if (display_frame) {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x,
...
bool ImGui::Selectable(const char *label, bool selected, ImGuiSelectableFlags flags,
    ...
    if (flags &
        ImGuiSelectableFlags_NoHoldingActiveID) { button_flags |= ImGuiButtonFlags_NoHoldingActiveId; }
    if (flags &
        ImGuiSelectableFlags_SelectOnClick) { button_flags |= ImGuiButtonFlags_PressedOnClick; }
    if (flags &
        ImGuiSelectableFlags_SelectOnRelease) { button_flags |= ImGuiButtonFlags_PressedOnRelease; }
    if (flags & ImGuiSelectableFlags_AllowDoubleClick) {
        button_flags |= ImGuiButtonFlags_PressedOnClickRelease |
                        ImGuiButtonFlags_PressedOnDoubleClick;
    }
    if (flags &
        ImGuiSelectableFlags_AllowItemOverlap) { button_flags |= ImGuiButtonFlags_AllowItemOverlap; }
...
bool
ImGui::ListBox(const char *label, int *current_item, const char *const items[], int items_count,
               int height_items) {
    ...
    if (values_count >= values_count_min) {
        ...
        for (int n = 0; n < res_w; n++) {
            ...
            ImVec2 pos1 = ImLerp(inner_bb.Min, inner_bb.Max,
                                 (plot_type == ImGuiPlotType_Lines) ? tp1 : ImVec2(tp1.x,
...
void
ImGui::PlotHistogram(const char *label, const float *values, int values_count, int values_offset,
                     const char *overlay_text, float scale_min, float scale_max, ImVec2 graph_size,
                     int stride) {
    ImGuiPlotArrayGetterData data(values, stride);
...
void
ImGui::PlotHistogram(const char *label, float (*values_getter)(void *data, int idx), void *data,
...
bool ImGui::BeginMenuBar() {
    ...
    ImRect clip_rect(IM_ROUND(bar_rect.Min.x + window->WindowBorderSize),
                     IM_ROUND(bar_rect.Min.y + window->WindowBorderSize), IM_ROUND(
                             ImMax(bar_rect.Min.x, bar_rect.Max.x - ImMax(window->WindowRounding,
...
bool ImGui::BeginViewportSideBar(const char *name, ImGuiViewport *viewport_p, ImGuiDir dir,
    ...
    if (bar_window == NULL || bar_window->BeginCount == 0) {
        ...
        ImGuiViewportP *viewport = (ImGuiViewportP *) (void *) (viewport_p ? viewport_p
                                                                           : GetMainViewport());
...
bool ImGui::BeginMainMenuBar() {
    ...
    g.NextWindowData.MenuBarOffsetMinVal = ImVec2(g.Style.DisplaySafeAreaPadding.x,
                                                  ImMax(g.Style.DisplaySafeAreaPadding.y -
                                                        g.Style.FramePadding.y, 0.0f));
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled) {
        ...
        ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        ImGuiWindow *child_menu_window = (g.BeginPopupStack.Size < g.OpenPopupStack.Size &&
                                          ...
                                          window) ? g.OpenPopupStack[g.BeginPopupStack.Size].Window
                                                  : NULL;
...
bool ImGui::MenuItemEx(const char *label, const char *icon, const char *shortcut, bool selected,
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        if (selected)
            RenderCheckMark(window->DrawList, pos + ImVec2(offsets->OffsetMark + stretch_w +
                                                           g.FontSize * 0.40f,
...
static inline int TabItemGetSectionIdx(const ImGuiTabItem *tab) {
    return (tab->Flags & ImGuiTabItemFlags_Leading) ? 0 : (tab->Flags & ImGuiTabItemFlags_Trailing)
                                                          ? 2 : 1;
}
...
void ImGui::EndTabBar() {
    ...
    g.CurrentTabBar = g.CurrentTabBarStack.empty() ? NULL : GetTabBarFromTabBarRef(
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar) {
    ...
    sections[1].Spacing =
            sections[1].TabCount > 0 && sections[2].TabCount > 0 ? g.Style.ItemInnerSpacing.x
                                                                 : 0.0f;
    ...
    if (width_excess > 0.0f && ((tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown) ||
        ...
        int shrink_data_offset = (central_section_is_visible ? sections[0].TabCount +
    ...
    if (tab_bar->ScrollingAnim != tab_bar->ScrollingTarget) {
        ...
        tab_bar->ScrollingAnim = teleport ? tab_bar->ScrollingTarget : ImLinearSweep(
        ...
        if (ImGuiTabItem *tab_item = TabBarFindTabByID(tab_bar, tab_bar->SelectedTabId)) {
            ...
            while (tab_to_scroll_to == NULL) {
                ...
                tab_to_scroll_to = &tab_bar->Tabs[(target_order >= 0 &&
                                                   target_order < tab_bar->Tabs.Size) ? target_order
                                                                                      : selected_order];
...
bool
ImGui::TabItemEx(ImGuiTabBar *tab_bar, const char *label, bool *p_open, ImGuiTabItemFlags flags) {
    ...
    const ImU32 tab_col = GetColorU32(
            (held || hovered) ? ImGuiCol_TabHovered : tab_contents_visible ? (tab_bar_focused
                                                                              ? ImGuiCol_TabActive
                                                                              : ImGuiCol_TabUnfocusedActive)
                                                                           : (tab_bar_focused
                                                                              ? ImGuiCol_Tab
...
ImVec2 ImGui::TabItemCalcSize(const char *label, bool has_close_button) {
    ...
    if (has_close_button)
        size.x += g.Style.FramePadding.x + (g.Style.ItemInnerSpacing.x +
...
void
ImGui::TabItemLabelAndCloseButton(ImDrawList *draw_list, const ImRect &bb, ImGuiTabItemFlags flags,
